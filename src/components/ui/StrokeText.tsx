...
// `gsap/ScrollTrigger` (~110KB, minificado) só é importado dinamicamente
// dentro do efeito, quando `trigger="scroll"` é realmente usado — um
// `import` estático no topo do arquivo puxava o plugin inteiro para o
// bundle de qualquer página que renderize este componente, mesmo com
// `trigger="mount"`/`"hover"`/`"loop"`, que nunca o usam.
...
interface StrokeTextProps {
  /** Texto renderizado como glifos SVG medidos — evite frases longas, o traçado é por caractere. */
  text?: string;
  strokeColor?: string;
  fillColor?: string;
  strokeWidth?: number;
...
/**
 * Wordmark decorativo desenhado à mão via SVG + GSAP: o contorno de cada
 * caractere se traça (stroke-dasharray/dashoffset) e depois o preenchimento
 * entra por cima (fade ou wipe da esquerda pra direita). Porte do componente
 * "StrokeText" da React Bits para TypeScript, mantendo a mesma API.
 * `prefers-reduced-motion` pula direto para o estado final (sem laço de
 * animação), mesmo cuidado dos outros componentes decorativos do site.
 */
export function StrokeText({
  ...
  useEffect(() => {
    ...
    const targets: unknown[] = [...strokes, ...fills, wipe].filter(Boolean);
  ...
  return (
    <span
      ref={rootRef}
      className={cn('stroke-text', trigger === 'hover' && 'stroke-text--hover', className)}
